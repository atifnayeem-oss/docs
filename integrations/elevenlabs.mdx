...
<Steps>
  <Step title="Connect ElevenLabs, once for your account">
    ...
    ElevenLabs API key. Ringup uses it to list your agents and provision the payment
  ...
  <Step title="Pick the agents that take payments">
    ...
    greeting, so a returning caller is greeted by name), the `create_checkout` server tool,
    and the transfer tool. Paste the one prompt line the dashboard
