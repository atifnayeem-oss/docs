...
Ringup. There is no SDK to install and no platform plugin to configure: your agent calls the REST
API directly, then hands the caller to Ringup's hosted payment line, which handles the card
conversation, consent, the charge, and the spoken confirmation.
...
<Steps>
  ...
  <Step title="Connect a test merchant">
    In the dashboard, add a merchant and connect its payment processor. In test mode this takes
    a minute and needs no real merchant account. The `merchant_id` it gives you (like
...
When the order is settled, create the Checkout Session. This one response carries everything
your agent needs to finish the call: the resolved total, whether payment is required, the exact
line to speak, and both transfer destinations.
...
This is the only decision in the integration, and you make it once, when you set your platform
...
<ParamField path="transfer_to_number" type="E.164 phone number">
  ...
  which Ringup stored when you called `create_checkout`. Requires the caller's number to survive
  your platform's transfer, which nearly all number transfers preserve.
</ParamField>

Both fields are present on every response where payment is required, so nothing about your code
changes if you switch platforms later. Speak `agent_message`, transfer to your chosen
destination, and your agent's job is done: the hosted line takes the payment, announces the
confirmation, and ends or returns the call.
...
The moment the payment lands (or fails), Ringup tells you two ways. Use whichever fits your
architecture; both carry the same facts.

**Webhook** (push): subscribe an endpoint in the dashboard and receive
...
dashboard. The endpoints, the fields, and your code stay exactly the same: the key selects the
