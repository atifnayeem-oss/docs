...
On ConversationRelay the wiring is code in your server, so the dashboard path covers the
account side: connecting the merchant, rehearsing in test mode, and going live. The calls
themselves are written in your server either way; they are the ones in the **Using the API**
tab.

<Steps>
  <Step title="Add the payment">
    In the Ringup dashboard, open your merchant, then **Add payment**, and pick Twilio
    ConversationRelay.
  </Step>
  <Step title="Wire your server">
    Add the identify call, the Checkout Session, and the transfer from the **Using the API**
    tab. The wiring lives in your server code, so it is identical on both paths.
  </Step>
  <Step title="Rehearse before you flip live">
    With the merchant in test mode, call your agent, place an order, and pay with the test
    card. The whole flow runs against the sandbox; no real card, no real charge. See
    [Testing](/concepts/testing).
  </Step>
  <Step title="Go live">
