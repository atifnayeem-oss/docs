...
On the Realtime API the wiring is code in your bridge, so the dashboard path covers the
account side: connecting the merchant, rehearsing in test mode, and going live. The calls
themselves are written in your bridge either way; they are the ones in the **Using the API**
tab.

<Steps>
  <Step title="Add the payment">
    In the Ringup dashboard, open your merchant, then **Add payment**, and pick OpenAI
    Realtime.
  </Step>
  <Step title="Wire your bridge">
    Add the identify call, the Checkout Session, and the transfer from the **Using the API**
    tab. The wiring lives in your bridge code, so it is identical on both paths.
  </Step>
