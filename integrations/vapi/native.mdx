...
<Note>
Ringup's pre-built squad calls `identify` at the greeting, so a returning caller is recognized
and greeted by name with no re-asking (for example, the agent never asks "what name for the
order?" when recognition already returned it). You do not wire this: it is built into the squad.
