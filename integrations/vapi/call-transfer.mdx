...
web checkout (see [Call transfer](/concepts/checkout-session)). Reach for it when you are
...
|---|---|
| `create_checkout` | An apiRequest tool on your agent that creates a Checkout Session and returns where to transfer |
| The transfer target | The `transfer_to` SIP URI the Checkout Session returns |
| The transfer | Vapi's `transferCall` with a SIP `destination` and `sipHeaders` |
...
to have Ringup read the result and end. The `id` and `transfer_to` are what Step 4 uses.

## Step 4: Transfer, silently and with context
