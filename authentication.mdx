---
title: "Authentication"
description: "Ringup authenticates every API request with a secret API key. Get one from your dashboard and send it as a Bearer token."
---

Ringup authenticates every API request with a secret **API key**. You pass it in the
`Authorization` header on each call:

```bash
Authorization: Bearer <YOUR_RINGUP_KEY>
```

## Get your key

Create an API key in your Ringup dashboard, under **Developers, then API keys**. Keys come in two
kinds, and the same code works with either, so you promote by swapping the key:

- **Test keys** (`sk_test_...`) run against the sandbox: fake money, real flows, ready-made
  merchants and callers. See [Testing](/concepts/testing).
- **Live keys** (`sk_live_...`) run against your real merchants and settle on their own processor.

## Pass it on every request

Every Ringup call, `identify`, `create_checkout`, `charge`, and the rest, takes the key as a
Bearer token:

<CodeGroup>
```bash curl
curl -X POST https://api.ringup.dev/v1/create_checkout \
  -H "Authorization: Bearer <YOUR_RINGUP_KEY>" \
  -H "Content-Type: application/json" \
  -d '{ "merchant_id": "tonys-pizza", "caller_phone": "+14155550142", "amount_cents": 2500 }'
```

```javascript Node
const res = await fetch("https://api.ringup.dev/v1/create_checkout", {
  method: "POST",
  headers: {
    "Authorization": "Bearer <YOUR_RINGUP_KEY>",
    "Content-Type": "application/json",
  },
  body: JSON.stringify({
    merchant_id: "tonys-pizza",
    caller_phone: "+14155550142",
    amount_cents: 2500,
  }),
});
```

```python Python
import requests

res = requests.post(
    "https://api.ringup.dev/v1/create_checkout",
    headers={"Authorization": "Bearer <YOUR_RINGUP_KEY>"},
    json={
        "merchant_id": "tonys-pizza",
        "caller_phone": "+14155550142",
        "amount_cents": 2500,
    },
)
```
</CodeGroup>

In an agent integration you set the key once and it rides every call:

- **Hosted, from the dashboard:** Ringup writes your key into the payment tools it provisions for
  you, so it rides every call without you handling it.
- **In your agent (API):** the key lives in the `Authorization` header of the Ringup tools or HTTP
  calls your agent makes, next to the other tool config.

Either way the key sits on your server or in the platform's tool configuration, never in a value
the model generates.

<Warning>
Your secret key can create charges. Keep it server-side. Never put it in client-side code, a
prompt, a transcript, or a public repository, and never let the model see or type it. If a key
leaks, roll it from the dashboard.
</Warning>

## Trying it without a key

The public demo sandbox exposes a seeded caller and demo merchants that need no key, so you can
run the [Quickstart](/quickstart) and hear a wired demo before you sign up. The moment you point
at your own merchants and callers, you use your key.

## Next steps

<CardGroup cols={2}>
  <Card title="Testing" icon="flask" href="/concepts/testing">
    The sandbox fixtures your test key (or no key) runs against.
  </Card>
  <Card title="How it works" icon="diagram-project" href="/concepts/how-it-works">
    The architecture, then the integration guide for your platform.
  </Card>
</CardGroup>
