...
cards and charge, so you write one parser. A declined charge arrives as a `200` with
`"status": "failed"` and carries these same fields under `failure`. Branch on `type` first,
then refine on `code`.
