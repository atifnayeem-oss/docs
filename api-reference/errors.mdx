...
**One envelope, every endpoint.** The same object comes back from provisioning, identify, checkout,
cards and charge, so you write one parser. A declined charge is no exception: it arrives as a
`402` in this same shape. Branch on `type` first, then refine on `code`.
...
<ResponseField name="audience" type="string">
  Who this error is for: `developer` or `caller`. It decides whether the three fields below
  carry anything. A bug in your integration is not something a caller can act on, so we do not
  hand your agent words for it.
</ResponseField>
...
<ResponseField name="agent_action" type="string | null">
  ...
  This list is deliberately short. Whether to escalate, end the call, or carry on without
  payment is a call-flow decision that belongs to you, not to your payments API, so we no
  longer return one.
</ResponseField>
...
error a CALLER can act on carries `agent_message`: one sentence, written to be spoken.

Check `audience` first. A `developer` error (a malformed request, a bad key, a merchant id that
does not exist) returns `null` for `agent_message`, `display_message` and `agent_action`,
because a caller cannot do anything about your integration and inventing a line for your agent to
say would be us guessing at your call flow.
...
## Declined payments are errors

A decline returns **`402`** with the same error envelope as everything else, carrying
`decline_code`, `advice_code` and `processor_code`. It is not a `200` with a status field,
so the ordinary `if (!response.ok)` check catches a declined payment rather than reading it as a
success.

Reading a payment *afterwards* is different: `GET /v1/checkout_sessions/{id}` is a lookup and returns `200` with
...
A decline arrives as a `402` from `charge`, in the standard envelope:
...
Branch on `decline_code`: it is Ringup's own vocabulary, identical no matter which processor the
merchant runs. `processor_code` carries that processor's raw value, unmodified, for your logs when
you need to trace a specific decline back to its source.
...
Each decline also carries `advice_code`, our guidance on what to do programmatically. It is
separate from `agent_action`, which drives the conversation: a card can be "do not retry this
card" while the agent still offers a different one.
