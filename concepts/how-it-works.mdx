...
hands the agent their name at the greeting, so the agent personalizes the whole call: it greets
them by name and skips re-asking for anything recognition already provides. Which cards they can
pay with comes later, from the [Checkout Session](/concepts/checkout-session), because that is
where the merchant is known. See
...
- **Hosted.** Ringup takes the payment on its own line. Your agent hands the call off at the payment
  ...
  from the dashboard. Your agent transfers the call to a Ringup payment line (works on any platform,
  see [Checkout Session](/concepts/checkout-session)).
- **In your agent (API).** Your agent stays on the call the whole time and calls Ringup's endpoints
  to add payment: [`identify`](/concepts/identity) at the greeting, then a
  ...
  [`send_link`](/concepts/send-link) when the session offers none. Nothing transfers, and because a
  recognized caller never enters a card, no card data ever touches your agent. The lighter, more
  flexible path.
