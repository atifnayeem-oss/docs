...
<Note>
The tool surface above always answers `200` and puts the outcome in `status`, because a voice
platform handles a non-`200` tool response poorly. Calling the REST API directly is different: a
decline there is a `402` in the standard [error envelope](/api-reference/errors). Either way the
reason and the sentence your agent can speak come back with it.
</Note>
