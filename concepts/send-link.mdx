...
phone, without ever reading a card number aloud on the call. It is the first-time path in both
integration modes: whether Ringup runs the hosted payment or your own agent calls the tools,
a first-time caller is enrolled the same way, and is [recognized](/concepts/identity) on every
call after.
